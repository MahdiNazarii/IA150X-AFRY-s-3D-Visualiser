...
public class FleetLiveServerConnection : MonoBehaviour
{
    ...
    public Dictionary<float, float[]> HashMap { get; private set; } = new Dictionary<float, float[]>();
    ...
    private bool MachinePositionUpdate(IList<object> machine)
    {
        try
        {   
            float key = (float)machine[2];
            float[] value = new float[] {(float)machine[3],(float)machine[4],(float)machine[5], (float)machine[10]};
            HashMap[key] = value;
            newDataFlag = true;
